...
const ComprobanteCliente = ({ compra, handleCloseModal, open }) => {
  ...
  useEffect(() => {
    const exportToPDF = async () => {
      const content = document.getElementById("compraComprobante");
      ...
      pdf.save(`factura_${compra.id_compra}.pdf`);
  ...
  return (
    <Modal open={open} onClose={handleCloseModal}>
      <div style={modalStyle}>
        <div id="compraComprobante" style={contentStyle} className="compra-info-modal">
            ...
            <table style={{ ...tableStyle, border: "1px solid #dddddd", marginTop: '10px' }}>
              <thead>
                <tr>
                  ...
                  <th style={thSubtotalStyle}>Valor de compra</th>
                </tr>
              </thead>
              <tbody>
                {compra.detalles.map((detalle) => (
                  <tr key={detalle.id_compra}>
                    <td style={tdStyle}>{detalle.fk_insumo}</td>
                    <td style={tdStyle}>{detalle.insumo.nombre} x {detalle.cantidad === 1
...
<p style={{ textAlign: 'right', margin: '8px 0' }}>
  <strong style={{ color: 'black', fontWeight: 'bold', fontFamily: 'Candara' }}>Valor a pagar:</strong> {formatearPrecios(compra.total)}
</p>
<hr />
<div style={{ display: 'flex', justifyContent: 'space-between', textAlign: 'left' }}>
  <div>
    ...
    <br />
    Referencia de compra: {compra.id_compra}
    <br />
    Fecha: {compra.fecha}
    <br />
